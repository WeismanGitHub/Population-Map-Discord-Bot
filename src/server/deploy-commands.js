...
const commandsPaths = getPaths(join(__dirname, 'dist\\server\\commands')).filter((file) =>
    file.endsWith('.js')
);
...
rest.put(Routes.applicationCommands(process.env.BOT_ID), { body: globalCommands }).then((res) => {
